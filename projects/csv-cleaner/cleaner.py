...
def clean_dataframe(
    ...
    *,
    ...
    Works for data loaded from both CSV and Excel files.
    """
    rows_in = len(df)
    ...
    # 3) Remove fully empty rows (all columns empty).
    # Cells are already normalized strings at this point, so a vectorized
    # comparison against "" is both correct and efficient.
    before_empty = len(df)
